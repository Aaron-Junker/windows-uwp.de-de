...
1. Vergewissern Sie sich, dass Python3 bereits installiert ist, indem Sie das Ubuntu-Terminal öffnen und Folgendes eingeben: `python3 --version`. Dies sollte Ihre python-Versionsnummer zurückgeben. Wenn Sie Ihre Python-Version aktualisieren müssen, aktualisieren Sie zunächst die Ubuntu-Version, indem Sie Folgendes eingeben: `sudo apt update && sudo apt upgrade`, und aktualisieren Sie anschließend python mithilfe von `sudo apt upgrade python3`.

2. Installieren Sie **PIP** , indem Sie Folgendes eingeben: `sudo apt install python3-pip`. PIP ermöglicht Ihnen, zusätzliche Pakete zu installieren und zu verwalten, die nicht Teil der python-Standardbibliothek sind.

3. Installieren Sie **venv** , indem Sie Folgendes eingeben: `sudo apt install python3-venv`.
...
1. Öffnen Sie den Projektordner in vs Code aus Ihrem Ubuntu-Terminal, indem Sie Folgendes eingeben: `code .` ("." teilt vs Code, den aktuellen Ordner zu öffnen).

2. Eine Sicherheitswarnung wird von Windows Defender angezeigt, und wählen Sie "Zugriff zulassen" aus. Nachdem vs Code geöffnet wurde, sollte der Host Indikator für Remote Verbindungen in der unteren linken Ecke angezeigt werden, in dem Sie wissen, dass Sie auf **wsl bearbeiten: Ubuntu-18.04 @ no__t-0.
...
1. Öffnen Sie das Fenster vs Code Datei-Explorer, indem Sie **STRG + UMSCHALT + E** eingeben (oder über das Menü zu **Ansicht** > -**Explorer**navigieren).
...
3. Erstellen Sie eine python-Datei, indem Sie Folgendes eingeben: `touch test.py`. Sie sollten sehen, dass die soeben erstellte Datei in Ihrem Explorer-Fenster unter den Ordnern. venv und. vscode bereits in Ihrem Projektverzeichnis angezeigt wird.
...
2. Erstellen Sie ein Verzeichnis für Ihr Projekt: `mkdir HelloWorld-Flask`, und `cd HelloWorld-Flask`, um das Verzeichnis einzugeben.
...
6. Aktivieren Sie die virtuelle Umgebung, die Sie in Schritt #3 mithilfe Ihres bash-Terminals in vs Code erstellt haben: `source .venv/bin/activate`. Wenn es funktioniert hat, sollte vor der Eingabeaufforderung (. venv) angezeigt werden.

7. Installieren Sie Flask in der virtuellen Umgebung, indem Sie Folgendes eingeben: `python3 -m pip install flask`. Vergewissern Sie sich, dass es installiert ist, indem Sie Folgendes eingeben: `python3 -m flask --version`.
...
14. Öffnen Sie den Standard Webbrowser für die gerenderte Seite, und **Klicken Sie** auf die http://127.0.0.1:5000/ -URL im Terminal. In Ihrem Browser sollte die folgende Meldung angezeigt werden:
...
2. Erstellen Sie ein Verzeichnis für Ihr Projekt: `mkdir HelloWorld-Django`, und `cd HelloWorld-Django`, um das Verzeichnis einzugeben.
...
7. Installieren Sie django in der virtuellen Umgebung mit dem folgenden Befehl: `python3 -m pip install django`. Vergewissern Sie sich, dass es installiert ist, indem Sie Folgendes eingeben: `python3 -m django --version`.
...
8. Führen Sie als nächstes den folgenden Befehl aus, um das Django-Projekt zu erstellen:
    ...
    Der Befehl "`startproject`" geht davon aus, dass es sich bei dem aktuellen Ordner um den Projektordner handelt (`.` am Ende), und er erstellt Folgendes:

    - `manage.py`: Das Befehlszeilen-Verwaltungs Programm Django für das Projekt. Sie führen administrative Befehle für das Projekt mit `python manage.py <command> [options]` aus.
    ...
    - Ein Unterordner mit dem Namen "`web_project`", der die folgenden Dateien enthält:
        ...
        - `wsgi.py`: ein Einstiegspunkt für wsgi-kompatible Webserver, um Ihr Projekt zu verarbeiten. Sie lassen diese Datei in der Regel unverändert, da Sie die Hooks für produktionsweb Server bereitstellt.
...
9. Um das Django-Projekt zu überprüfen, starten Sie den Entwicklungs Server von Django mithilfe des Befehls `python3 manage.py runserver`. Der Server wird auf dem Standardport 8000 ausgeführt, und es sollte eine Ausgabe wie die folgende Ausgabe im Terminalfenster angezeigt werden:
    ...
    Wenn Sie den Server erstmalig ausführen, wird eine SQLite-Standarddatenbank in der Datei `db.sqlite3` erstellt, die für Entwicklungszwecke vorgesehen ist, aber in der Produktionsumgebung für Web-Apps mit geringem Volumen verwendet werden kann. Außerdem ist der integrierte Webserver von Django *nur* für lokale Entwicklungszwecke vorgesehen. Wenn Sie jedoch auf einem Webhost bereitstellen, verwendet Django stattdessen den Webserver des Hosts. Das Modul "`wsgi.py`" im Django-Projekt übernimmt die Einbindung in die Produktionsserver.

    Wenn Sie einen anderen Port als den Standardwert 8000 verwenden möchten, geben Sie die Portnummer in der Befehlszeile an, z. b. `python3 manage.py runserver 5000`.

10. `Ctrl+click` die URL des `http://127.0.0.1:8000/` im Terminal Ausgabefenster, um den Standardbrowser für diese Adresse zu öffnen. Wenn Django ordnungsgemäß installiert ist und das Projekt gültig ist, wird eine Standardseite angezeigt. Im Fenster vs Code Terminal Ausgabe wird auch das Server Protokoll angezeigt.

11. Wenn Sie fertig sind, schließen Sie das Browserfenster, und beenden Sie den Server in vs Code, indem Sie `Ctrl+C` verwenden, wie im Terminal Ausgabefenster angegeben.

12. Führen Sie nun zum Erstellen einer Django-App den Befehl "`startapp`" des Verwaltungs Programms in Ihrem Projektordner (in dem sich `manage.py` befindet) aus:

    ```bash
    python3 manage.py startapp hello
    ```

    Der Befehl erstellt einen Ordner namens "`hello`", der eine Reihe von Code Dateien und einen Unterordner enthält. Dabei arbeiten Sie häufig mit `views.py` (das die Funktionen enthält, die Seiten in Ihrer Web-App definieren) und `models.py` (das Klassen enthält, die ihre Datenobjekte definieren). Der Ordner "`migrations`" wird vom Verwaltungs Programm von Django zum Verwalten von Daten Bank Versionen verwendet, wie weiter unten in diesem Tutorial erläutert. Es gibt auch die Dateien `apps.py` (app-Konfiguration), `admin.py` (zum Erstellen einer administrativen Oberfläche) und `tests.py` (für Tests), die hier nicht behandelt werden.

13. Ändern Sie `hello/views.py` entsprechend dem folgenden Code, der eine einzelne Ansicht für die Startseite der App erstellt:
...
14. Erstellen Sie eine Datei (`hello/urls.py`) mit dem unten stehenden Inhalt. Die Datei `urls.py` gibt an, wo Sie Muster zum Weiterleiten verschiedener URLs an die entsprechenden Ansichten angeben. Der folgende Code enthält eine Route zum Zuordnen der Stamm-URL der APP (`""`) zur `views.home`-Funktion, die Sie soeben `hello/views.py` hinzugefügt haben:
...
15. Der Ordner "`web_project`" enthält auch eine Datei mit `urls.py`, bei der das URL-Routing tatsächlich behandelt wird. Öffnen Sie `web_project/urls.py`, und ändern Sie ihn so, dass er dem folgenden Code entspricht (Sie können die aufschlussreichen Kommentare bei Bedarf beibehalten). Dieser Code Ruft den `hello/urls.py` der App mithilfe `django.urls.include` ab, der die in der APP enthaltenen Routen der APP beibehält. Diese Trennung ist hilfreich, wenn ein Projekt mehrere apps enthält.
...
17. Führen Sie im vs Code Terminal den Development Server mit `python manage.py runserver` aus, und öffnen Sie einen Browser mit `http://127.0.0.1:8000/`, um eine Seite anzuzeigen, die "Hello, Django" rendert.
