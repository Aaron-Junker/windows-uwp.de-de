...
Some properties on objects cannot be set in XAML by any means, and can only be set using code. In der Regel sind dies Eigenschaften, die sich besser für CodeBehind eignen als für XAML.
...
Die Sammlungssyntax in XAML vermittelt den Eindruck, dass Sie eine schreibgeschützte Eigenschaft festlegen, was jedoch nicht der Fall ist. See "<bpt id="p1">[</bpt>Collection Syntax<ept id="p1">](#collection-syntax)</ept>" later in this topic.
...
Viele Sammlungseigenschaften werden auch als XAML-Inhaltseigenschaft für die Klasse identifiziert. Die Kombination aus impliziter Sammlungsverarbeitung und XAML-Inhaltssyntax ist oft bei Typen zu finden, die zum Zusammensetzen von Steuerelementen wie Panels, Ansichten oder ItemsControls verwendet werden. For example, the following example shows the simplest possible XAML for compositing two peer UI elements within a <bpt id="p1">[</bpt><bpt id="p2">**</bpt>StackPanel<ept id="p2">**</ept><ept id="p1">](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.StackPanel)</ept>.
...
Zunächst gibt das Vorhandensein des Objektelements an, dass ein neues *objectName*-Objekt instanziiert werden muss. Only after such an instance exists can the instance property <bpt id="p1">*</bpt>propertyName<ept id="p1">*</ept> be set on it.
...
<bpt id="p1">**</bpt>Note<ept id="p1">**</ept><ph id="ph1">  </ph>In reference topics for APIs that use XAML and use enumerations, click the link to the enumeration type in the <bpt id="p2">**</bpt>Property value<ept id="p2">**</ept> section of <bpt id="p3">**</bpt>Syntax<ept id="p3">**</ept>. So gelangen Sie auf die Enumerationsseite, auf der Sie die benannten Konstanten für diese Enumeration ermitteln können.
