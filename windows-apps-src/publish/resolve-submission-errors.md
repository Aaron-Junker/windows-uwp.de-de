...
-   **Mindestens eine Assembly im Paket wird falsch verdeckt:** Verwenden Sie ein anderes Tool, um die Verschleierung auszuführen, oder entfernen Sie die Verschleierung. Der Kompilierungsprozess optimiert die verborgenen Assemblys, aber gelegentlich werden jedoch einige Assemblys mit einem Tool verborgen, das die MSIL auf nicht unterstützt Weise verändert und daher einen Fehler auslöst.
-   **Die Größe von mindestens einer Methode in der APP überschreitet 256 KB von Il:** Umgestalten der problematischen Methode in kleinere Funktionen. Die MSIL-Größe für Methoden in einer Assembly kann mit dem ILDASM-Tool ermittelt werden.
-   **Fehler bei der Überprüfung der starken Namens Signatur für eine oder mehrere Assemblys:** Dieser Fehler tritt normalerweise auf, wenn die Signierung mit starkem Namen mit einem anderen Schlüssel als dem in den Assemblymetadaten erwarteten Vorgang durchgeführt wurde. Verwenden Sie den richtigen Schlüssel zum Signieren, oder entfernen Sie die starke Namenssignatur.
-   **Das Paket enthält die Assemblys "gemischter Modus" (mit verwaltetem und nativem Code):** Assemblys mit gemischtem Modus werden auf Windows Phone nicht unterstützt. Entfernen Sie die betreffenden Assemblys aus dem Paket, und reichen Sie die App erneut ein.
-   **Eine Windows Phone 8,1 XAP-oder AppX-/appxbundle-Assembly ist ungültig:** Stellen Sie sicher, dass die winmd-Datei mindestens einen öffentlichen Einstiegspunkt enthält. Sie können ggf. eine beliebige Dekompilierungsanwendung verwenden, um den Code zu überprüfen und öffentliche Einstiegspunkte zu suchen.
...
Wenn eine Fehlermeldung angezeigt wird, **die besagt, dass der im Paket gefundene Name keinem Ihrer reservierten APP-Namen entspricht. Reservieren Sie den APP-Namen, und/oder aktualisieren Sie Ihr Paket mit dem richtigen APP-** Namen für diese Sprache. Dies liegt möglicherweise daran, dass Sie einen falschen Namen in Ihr Paket eingegeben haben. Dieser Fehler kann auch auftreten, wenn Sie einen APP-Namen verwenden, den Sie nicht im Partner Center reserviert haben. In der Regel können Sie diesen Fehler beheben, indem Sie folgende Schritte ausführen:
