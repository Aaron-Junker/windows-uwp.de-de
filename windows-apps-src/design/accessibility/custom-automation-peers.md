...
* Steuerelement Muster für Benutzeroberflächen Automatisierung: Ein Steuerelement kann mehr als ein Muster unterstützen, von denen jedes eine andere Klassifizierung von Informationen oder Interaktionen darstellt. Jeder Steuerelementtyp verfügt über einen Satz von Steuerelementmustern, die das Steuerelement unterstützen muss. Dabei ist ein Satz optional, und der andere darf vom Steuerelement nicht unterstützt werden.
* Eigenschaftswerte der Benutzeroberflächen Automatisierung: Jeder Steuer ungstyp verfügt über eine Reihe von Eigenschaften, die vom Steuerelement unterstützt werden müssen. Dabei handelt es sich um die allgemeinen Eigenschaften, die unter [Übersicht über Eigenschaften für die Benutzeroberflächenautomatisierung](https://docs.microsoft.com/windows/desktop/WinAuto/uiauto-propertiesoverview) beschrieben werden, nicht um die musterspezifischen Eigenschaften.
* Benutzeroberflächenautomatisierungs-Ereignisse: Jeder Steuer ungstyp verfügt über eine Reihe von Ereignissen, die vom Steuerelement unterstützt werden müssen. Auch hier handelt es sich nicht um musterspezifische Ereignisse, sondern um die unter [Übersicht über Ereignisse für die Benutzeroberflächenautomatisierung](https://docs.microsoft.com/windows/desktop/WinAuto/uiauto-eventsoverview) beschriebenen Ereignisse.
* Benutzeroberflächenautomatisierungs-Struktur: Jeder Steuer Steuerungstyp definiert, wie das Steuerelement in der Benutzeroberflächenautomatisierungs-Struktur angezeigt werden muss.
...
Wenn Sie Steuerelement Muster für ein Framework mit verwaltetem Code implementieren, z. b. eine UWP-App mit C @ no__t-0 oder Microsoft Visual Basic, können Sie diese Muster mithilfe .NET Framework Schnittstellen anstelle der COM-Schnittstellen Darstellung darstellen. Beispielsweise ist die Benutzeroberflächenautomatisierungs-Musterschnittstelle für eine Microsoft .NET-Anbieterimplementierung des **Invoke**-Musters [**IInvokeProvider**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Automation.Provider.IInvokeProvider).
...
[**Getboundingrechglecore**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.automation.peers.automationpeer.getboundingrectanglecore) und [**GetClickablePointCore**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.automation.peers.automationpeer.getclickablepointcore) werden hauptsächlich für automatisierte Testszenarien verwendet. Wenn Sie automatisierte Tests für Ihr Steuerelement unterstützen möchten, sollten Sie diese Methoden überschreiben. Dies kann für Bereichssteuerelemente nützlich sein, bei denen Sie nicht nur einen einzelnen Punkt vorschlagen können, weil sich abhängig von der Klickposition des Benutzers im Koordinatenbereich jeweils eine andere Auswirkung auf einen Bereich ergibt. Beispielsweise überschreibt der standardmäßige [**ScrollBar**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Primitives.ScrollBar)-Automatisierungspeer **GetClickablePointCore**, um einen „keine Zahl“-Wert des Typs [**Point**](https://docs.microsoft.com/uwp/api/Windows.Foundation.Point) zurückzugeben.

[**Getlivesettingcore**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.automation.peers.automationpeer.getlivesettingcore) wirkt sich auf den Standardwert des Steuer Elements für den **livesesetting** -Wert der Benutzeroberflächen Automatisierung aus. Sie sollten dies überschreiben, wenn das Steuerelement einen anderen Wert als [**AutomationLiveSetting.Off**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Automation.Peers.AutomationLiveSetting) zurückgeben soll. Weitere Informationen zu **LiveSetting** finden Sie unter [**AutomationProperties.LiveSetting**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.automation.automationproperties.livesettingproperty).
...
* [**Iskeyboardfocus ablecore**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.automation.peers.automationpeer.iskeyboardfocusablecore): Gibt **true** zurück, wenn der Besitzer ein [**Steuer**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Control)Element ist. Andernfalls ist Sie **false**.
