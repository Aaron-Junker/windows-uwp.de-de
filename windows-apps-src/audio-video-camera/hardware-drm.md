...
-   Windows Media Video (auch als VC-1 bezeichnet) wird nicht unterstützt (siehe über [Schreiben von Hardware-DRM](#override-hardware-drm)).
...
Die beste Möglichkeit, um zu ermitteln, ob Sie sich in Hardware DRM oder Software DRM befinden, ist das Untersuchen von C: \\ Benutzer \\ &lt; Benutzername &gt; \\ AppData \\ lokale \\ Pakete \\ &lt; Anwendungsname &gt; \\ LocalCache \\ PlayReady.\\\*

-   Wenn dort die Datei „mspr.hds“ vorhanden ist, verwenden Sie ein softwarebasiertes DRM.
-   Wenn Sie über eine andere \* HDS-Datei verfügen, befinden Sie sich in Hardware DRM.
...
Sie können die [**PlayReadyStatics.CheckSupportedHardware**](/uwp/api/windows.media.protection.playready.playreadystatics.checksupportedhardware)-Methode verwenden, um festzustellen, ob das System ein bestimmtes Hardware-DRM-Feature unterstützt. Beispiel:

```csharp
bool isFeatureSupported = PlayReadyStatics.CheckSupportedHardware(PlayReadyHardwareDRMFeatures.HEVC);
```

Die [**PlayReadyHardwareDRMFeatures**](/uwp/api/Windows.Media.Protection.PlayReady.PlayReadyHardwareDRMFeatures)-Enumeration enthält die Liste gültiger Werte für das Hardware-DRM-Feature, die abgefragt werden können. Um festzustellen, ob das Hardware-DRM unterstützt wird, verwenden Sie den **HardwareDRM**-Member in der Abfrage. Um festzustellen, ob die Hardware den High Efficiency Video Coding (HEVC)/H.265-Codec unterstützt, verwenden Sie den **HEVC**-Member in der Abfrage.

Sie können auch die [**PlayReadyStatics.PlayReadyCertificateSecurityLevel**](/uwp/api/windows.media.protection.playready.playreadystatics.playreadycertificatesecuritylevel)-Eigenschaft zum Abrufen der Sicherheitsstufe des Clientzertifikats verwenden, um festzustellen, ob das Hardware-DRM unterstützt wird. Sofern die zurückgegebene Zertifikatssicherheitsstufe nicht größer oder gleich 3000 ist, ist entweder der Client nicht individualisiert oder bereitgestellt (in diesem Fall gibt die Eigenschaft 0 zurück) oder das Hardware-DRM wird nicht verwendet (in diesem Fall gibt die Eigenschaft einen Wert unter 3000 zurück).

### <a name="detecting-support-for-aes128cbc-hardware-drm"></a>Erkennen der Unterstützung für AES128CBC Hardware DRM
Ab Windows 10, Version 1709, können Sie die Unterstützung für die AES128CBC-Hardware Verschlüsselung auf einem Gerät erkennen, indem Sie **[playleseystatics. checksupportedhardware](/uwp/api/windows.media.protection.playready.playreadystatics.checksupportedhardware)** aufrufen und den Enumerationswert [**playleseyhardwaredrmfeatures. AES128CBC**](/uwp/api/Windows.Media.Protection.PlayReady.PlayReadyHardwareDRMFeatures)angeben. In früheren Versionen von Windows 10 bewirkt die Angabe dieses Werts, dass eine Ausnahme ausgelöst wird. Aus diesem Grund sollten Sie überprüfen, ob der-Enumerationswert vorhanden ist, indem Sie **[apiinformation. isapiobjects Present](/uwp/api/windows.foundation.metadata.apiinformation.isapicontractpresent)** aufrufen und die Haupt Vertragsversion 5 vor dem Aufrufen von **checksupportedhardware**angeben.
